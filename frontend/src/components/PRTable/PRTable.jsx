 ...
 */
/** @param {{pr: PullRequest, stackView?: boolean}} props */
 ...
 *   onSortingChange: import('@tanstack/react-table').OnChangeFn<import('@tanstack/react-table').SortingState>,
 *   workspaceStates?: Map<string, string>,
 *   dismissedIdle?: Set<string>,
 *   stackView?: boolean,
 *   sortedRowsRef?: import('react').MutableRefObject<PullRequest[] | null>,
 * }} props
 */
export function PRTable({
  ...
  const columns = useMemo(
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => <StatusBadge status={row.original.draft ? 'draft' : 'open'} type="status" />,
        meta: { centered: true },
      },
      {
        id: 'ci_status',
        ...
        cell: ({ row }) => <StatusBadge status={row.original.ci_status} type="ci" />,
        meta: { centered: true },
      },
      {
        id: 'review_status',
        ...
        cell: ({ row }) => <StatusBadge status={row.original.review_status} type="review" />,
        meta: { centered: true },
      },
      {
        id: 'mergeable',
        ...
        cell: ({ row }) => <StatusBadge status={row.original.mergeable} type="merge" />,
        meta: { centered: true },
      },
      {
        accessorKey: 'updated_at',
        header: 'Updated',
        cell: ({ row }) => getRelativeTime(row.original.updated_at),
      ...
      },
      {
        ...
        meta: {
          alignRight: true,
          headerTitle: 'Weekday hours from PR creation to the first non-author, non-bot review or comment',
        },
      },
    ],
  ...
  return (
    <div className={styles.tableScroller} role="region" aria-label="Pull requests" tabIndex={0}>
